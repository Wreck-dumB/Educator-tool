      ...
      properties: {
        activities: {
          ...
          properties: {
            ...
            image_subject: {
              type: "string",
              description: "Set this whenever a craft/materials activity, a 'drawing_frame' activity, a 'name_colouring' activity, or a 'name_label' activity has one genuine, concrete, drawable subject — a short noun phrase an image generator can literally draw. The image generator draws animals, objects, and creatures well but consistently renders human children/people as unsettling realistic portraits, no matter the prompt — so NEVER describe a person, child, or generic 'character' (e.g. NOT 'a beloved storybook character'). Pick a concrete emblematic object or animal instead (e.g. 'a smiling sun', 'a friendly dinosaur', 'a cartoon fox in a scarf'). For 'name_colouring'/'name_label' specifically: only set this when the educator's request names or clearly implies a theme (e.g. 'name colouring sheet with a dinosaur', 'space-themed name sheet') — a plain 'colour in my name'/'name label' request with no theme should leave this OUT so the sheet prints with just the name. Leave OUT ENTIRELY whenever the activity is open-ended/free-choice with no specific depicted subject, or when using letter_colouring (never use image generation for letters/numbers). Never invent a subject just to fill this field.",
            },
            matching_left: {
              type: "array",
              items: { type: "string" },
