...
import { acceptChildInvite } from "./actions";
...
export default async function AcceptChildInvitePage({
  ...
  const { data: previewRows } = await supabase.rpc("get_child_invite_preview", { _token: token });
    ...
    return (
      <InviteShell>
        <p className="text-ink/70">
          This invite link doesn&apos;t exist. Double-check the link, or ask the educator who
  ...
  if (preview.status !== "pending") {
    ...
    return (
      <InviteShell>
        <p className="text-ink/70">
          This invite has {statusText}. Ask the educator who invited you to send a new one if you
          still need to link your account.
        </p>
      </InviteShell>
    );
  }
  ...
  if (new Date(preview.expires_at) <= new Date()) {
    return (
      <InviteShell>
        <p className="text-ink/70">This invite has expired. Ask the educator to send a new one.</p>
      </InviteShell>
    );
  }

  const nextPath = `/parent/accept-invite/${token}`;
    ...
    return (
      <InviteShell>
        ...
        <div className="mt-4 flex gap-3">
          ...
          <Link
            href={`/parent/signup?next=${encodeURIComponent(nextPath)}`}
  ...
  return (
    <InviteShell>
      ...
      <form action={acceptChildInvite} className="mt-4">
        <input type="hidden" name="token" value={token} />
        <button type="submit" className={`w-full ${primaryButtonClass}`}>
          Join {preview.child_first_name}&apos;s profile
        </button>
      </form>
    </InviteShell>
  );
