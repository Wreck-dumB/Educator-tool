...
export default async function ProgramCalendarPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // One printed A3 sheet per Mon–Fri week — a 4-week program prints as 4
  // pages, not just its first week, so the whole thing can go up in the room.
  const allWeekdayDates = eachDateInRange(program.start_date, program.end_date).filter(isWeekday);
  const weeks = chunk(allWeekdayDates, 5);
  ...
  return (
    <div className="mx-auto max-w-[1400px] px-4 py-6 print:px-0 print:py-0">
      ...
      {weeks.map((weekDates, weekIdx) => {
        const hasAnyUnsorted = weekDates.some((date) => unsortedEntriesFor(date).length > 0);
        const weekCulturalDays = program.cultural_days.filter(
          (d) => d.date >= weekDates[0] && d.date <= weekDates[weekDates.length - 1],
        );
        return (
          <div
            key={weekDates[0]}
            className="mt-4 rounded-3xl border-4 border-sage bg-cream p-6 print:mt-2 print:rounded-2xl print:border-2 print:border-black print:bg-white"
            style={weekIdx < weeks.length - 1 ? { breakAfter: "page" } : undefined}
          >
            <div className="flex items-center justify-between gap-4 border-b-2 border-sage/40 pb-4 print:border-black">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[0.2em] text-sage-dark print:text-black">
                  🌱 DR. SparkPlay — Weekly Program{weeks.length > 1 ? ` · Week ${weekIdx + 1} of ${weeks.length}` : ""}
                </p>
                ...
                <p className="mt-1 text-sm text-ink/70 print:text-black">
                  {new Date(weekDates[0]).toLocaleDateString(undefined, { day: "numeric", month: "long", year: "numeric" })}
                  {" – "}
                  {new Date(weekDates[weekDates.length - 1]).toLocaleDateString(undefined, { day: "numeric", month: "long", year: "numeric" })}
                  {room && <> &nbsp;·&nbsp; {room.name}</>}
                </p>
              </div>
            ...
            {weekCulturalDays.length > 0 && (
              <div className="mt-6 rounded-xl border-2 border-amber-300 bg-amber-light px-4 py-2 print:border-black print:bg-white">
                ...
                <p className="mt-1 text-sm text-amber-dark/90 print:text-black">
                  {weekCulturalDays.map((d) => `${d.name} (${d.date})`).join("  •  ")}
                </p>
              </div>
            )}
