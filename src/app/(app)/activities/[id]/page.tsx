...
export default async function ActivityDetailPage({
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      {(() => {
        const p = new URLSearchParams({ type: "activity_sheet", title: activity.title });
        activity.steps.slice(0, 6).forEach((s) => p.append("step", s));
        activity.materials_used.slice(0, 8).forEach((m) => p.append("material", m));
        return (
          <a
            href={`/worksheet?${p.toString()}`}
            target="_blank"
