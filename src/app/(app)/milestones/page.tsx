  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <p className="mt-1 text-sm text-ink/60">
        ...
        from birth to 18 years — separate from the EYLF outcomes, which are curriculum dispositions
        without expected ages. Birth-to-5 sourced from Australian health-system guidance (Sydney
        Children&apos;s Hospitals Network, healthdirect); 5-12 years from Emerging Minds (the
        Australian Government-funded National Workforce Centre for Child Mental Health); 12-18 years
        from the American Academy of Pediatrics, cross-checked against Australian puberty-timing data
        where direct AU teen-development sources weren&apos;t accessible to fetch automatically.
      </p>
      <p className="mt-2 rounded-xl bg-amber-light px-3 py-2 text-xs text-amber-dark">
        Every child and young person develops at their own pace — this is a general guide for
        planning, not a screening or diagnostic tool. If you have concerns about a child&apos;s
        development, refer to a GP, maternal &amp; child health nurse, paediatrician, school
        counsellor, or for older teens, headspace.
      </p>
        ...
        {ageBands.map((band) => (
          <div key={band} className={`p-4 ${cardClass}`}>
            ...
            <div className="mt-3 grid grid-cols-1 gap-4 sm:grid-cols-2">
              {(["physical", "gross_motor", "fine_motor", "language", "social_emotional", "cognitive"] as const).map(
                (domain) => {
